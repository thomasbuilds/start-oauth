...
const deriveKey = async (secret: string) =>
  subtle.deriveKey(
    ...
    await subtle.importKey("raw", Buffer.from(secret), "PBKDF2", false, [
      "deriveKey",
    ]),
...
export async function makeState(
  ...
  const verifier = Buffer.from(getRandomValues(new Uint8Array(32))).toString(
    "hex"
  );
  const digest = await subtle.digest("SHA-256", Buffer.from(verifier));
  ...
  const encrypted = await subtle.encrypt(
    ...
    Buffer.from(
      JSON.stringify({ fallback, redirect, exp: Date.now() + 180000, verifier })
    )
  );
